      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="py-12 grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-white uppercase tracking-wider">About ShoeMarshal</h3>
            <p className="text-sm">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {['Home', 'Bag', 'Products', 'Category' ].map((item) => (
                <li key={item}>
                  <Link href={`/shop/${item.toLowerCase().replace(' ', '-')}`} className="text-sm hover:text-white transition-colors">
                    {item}
                  </Link>
                </li>
              ))}
        ...
        <div className="border-t border-gray-800 pt-8 pb-6 flex flex-col md:flex-row justify-between items-center">
          <p className="text-sm">
            &copy; {new Date().getFullYear()} ShoeMarshal Anime Merch. All Rights Reserved.
          </p>
